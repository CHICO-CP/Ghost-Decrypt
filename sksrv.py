...
DECRYPTION_KEY = "6pq8YieK$8D2kT4a6Pizv3i56nWi"

# Función para descifrar archivos SKSRV
...
# Función para filtrar contenido SKSRV
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        print("Usage: python decrypt_sksrv.py <encrypted_sksrv_file>")
        sys.exit(1)

    encrypted_sksrv_file = sys.argv[1]

    with open(encrypted_sksrv_file, 'rb') as f:
        encrypted_sksrv = f.read()

    decrypted_content = decrypt_sksrv_file(encrypted_sksrv, DECRYPTION_KEY)
    if decrypted_content:
        filtered_content = filter_sksrv_content(decrypted_content)
